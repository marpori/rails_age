  ...
  context 'when using invoke' do
    ...
    context 'with a namespace' do
      ...
      it "inserts correct name and date" do
        ...
        type_content = File.read(types_config_path)
        expect(type_content)
          .to include("require_dependency 'test/test_node'")
        expect(type_content)
          .to include(":test_test_node, ApacheAge::Types::AgeTypeGenerator.create_type_for(Test::TestNode)")
        ...
        type_content = File.read(types_config_path)
        expect(type_content)
          .not_to include("require_dependency 'test/test_node'")
        expect(type_content)
          .not_to include(":test_test_node, ApacheAge::Types::AgeTypeGenerator.create_type_for(Test::TestNode)")
      end
    end
  end
end
